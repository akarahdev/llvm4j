...
public interface ControlFlowInstructionHook<
    T extends ControlFlowInstructionHook<T>
>
    ...
    default T br(
        Value condition,
        BasicBlock.BuildMapper then,
        BasicBlock.BuildMapper orElse
    ) {
        var bb1 = then
            .apply(new BasicBlock.Builder(this.functionBuilder()))
            .build();
        var bb2 = orElse
            .apply(new BasicBlock.Builder(this.functionBuilder()))
            .build();
    ...
    default Value callTyped(
        TypeValuePair functionPointer,
        List<TypeValuePair> arguments
    ) {
        var id = Identifier.localRandom();
        this.instructions().add(sc ->
            sc
                .append("{} = call {}", id, functionPointer)
                .append('(')
                .append(arguments, ", ")
                .append(')')
        );
