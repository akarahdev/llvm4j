...
public interface MemoryInstruction<T extends MemoryInstruction<T>>
    ...
    default Value load(Type type, Value pointer) {
        var id = Identifier.localRandom();
        this.instructions().add(sc ->
            sc.append("{} = load {}, ptr {}", id, type, pointer)
        );
        return id;
    }
    ...
    default Value load(Type type, Value pointer, LoadFlags... flags) {
        ...
        this.instructions().add(sc ->
            sc
                .append("{} = load ", id)
                .appendIf(
                    () -> flags2.contains(LoadFlags.ATOMIC),
                    sc2 -> sc2.append("atomic")
                )
                .appendIf(
                    () -> flags2.contains(LoadFlags.VOLATILE),
                    sc2 -> sc2.append("volatile")
                )
    ...
    default T store(
        ...
        this.instructions().add(sc ->
            sc
                .append("store ")
                .appendIf(
                    () -> flags2.contains(StoreFlags.ATOMIC),
                    sc2 -> sc2.append("atomic")
                )
                .appendIf(
                    () -> flags2.contains(StoreFlags.VOLATILE),
                    sc2 -> sc2.append("volatile")
                )
    ...
    default Value getElementPtr(
        Type type,
        Value pointer,
        TypeValuePair... indices
    ) {
        var id = Identifier.localRandom();
        this.instructions().add(sc -> {
